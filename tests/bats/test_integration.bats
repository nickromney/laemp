...
@test "full moodle installation with nginx and mysql" {
  ...
  # Run full Moodle installation (Moodle 5.2/stable502)
  run exec_laemp -p -w nginx -d mysql -m 502 -S -c
  echo "Output: $output"
  [ "$status" -eq 0 ]
...
@test "full moodle installation with apache and postgresql" {
  ...
  # Run full Moodle installation with Apache and PostgreSQL (Moodle 5.2/stable502)
  run exec_laemp -p -w apache -f -d pgsql -m 502 -S -c
  echo "Output: $output"
  [ "$status" -eq 0 ]
...
@test "moodle installation creates vhost configuration" {
  ...
  # Run Moodle installation (Moodle 5.2/stable502)
  run exec_laemp -p -w nginx -d mysql -m 502 -S -c
  [ "$status" -eq 0 ]
...
@test "php configuration is optimized for moodle" {
  ...
  run exec_laemp -p -w nginx -d mysql -m 502 -c
  [ "$status" -eq 0 ]
...
@test "full stack with all options" {
  ...
  run exec_laemp -p -w nginx -d mysql -m 502 -S -r -M -c
  echo "Output: $output"
  [ "$status" -eq 0 ]
